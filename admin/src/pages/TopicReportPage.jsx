...
import ReportNavbar from '../components/TestReportNavbar';
import { motion } from 'framer-motion';
import { PieChart, Pie, Cell, Tooltip, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Legend } from 'recharts';

const COLORS = ['#10B981', '#EF4444', '#3B82F6'];

const TopicReportPage = () => {
  ...
  const calculateTopicStats = (topic) => {
    ...
    const percentage = maxPossibleMarks > 0 ? Math.max((finalScore / maxPossibleMarks) * 100, 0).toFixed(2) : '0.00';
    const accuracy = stats.attempted > 0 ? ((stats.correct / stats.attempted) * 100).toFixed(2) : '0.00';
          ...
          {topics.map(topic => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium shadow-sm transition-all duration-200 ${
                activeTopic === topic ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700'
              }`}
            >
              {topic}
            </button>
