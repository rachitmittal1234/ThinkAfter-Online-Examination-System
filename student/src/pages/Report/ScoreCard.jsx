...
const ScoreCard = () => {
    ...
    const [resolvedTestId, setResolvedTestId] = useState(null);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const testId = reportData?.[0]?.test;
                if (!testId) return console.error('Test ID missing in report');
  ...
  <div className="flex justify-center mb-8">
    <a
      href={`${backendUrl}/api/questions/pdf/${resolvedTestId}`}
      download={`Test_${resolvedTestId}_Questions.pdf`}
      target="_blank"
