...
const QuestionReport = () => {
    ...
    const fetchReport = async () => {
      try {
        ...
        const reportRes = await axios.get(
          `${backendUrl}/api/reports/report/${userId}/${reportId}`,
          {
        ...
        // Then get the test details to find all questions
        const testId = reportRes.data.report?.[0]?.test;
        if (!testId) throw new Error('Test ID missing in report');
  ...
  const handleViewResponse = (entry) => {
    navigate(`/view-question-response/${entry.question._id}`, {
      state: entry
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-lg text-gray-600">Detailed analysis of your question-wise result</p>
        </motion.div>
        ...
        {/* <div className="mb-4 p-4 bg-yellow-50 rounded-lg">
          ...
          <p className="text-sm text-yellow-700">
            First item: {reportData[0] ? JSON.stringify(reportData[0]) : 'No data'}
          </p>
        </div> */}
